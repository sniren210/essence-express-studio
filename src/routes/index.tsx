...
const FAQS = [
  ...
  { q: "Apakah saya bisa menentukan budget campaign sendiri?", a: "Ya, Anda bebas menentukan budget dan persyaratan kampanye." },
  { q: "Apakah saya bisa meminta revisi konten?", a: "Setiap paket memiliki ketentuan revisi, biasanya 1-2 kali revisi gratis." },
  ...
  return (
    <Layout>
      ...
      <section className="relative px-6 pt-20 pb-16 text-center">
        ...
        <p className="mt-4 text-xs text-muted-foreground">Dipercaya oleh 500+ brand</p>
        <div className="mx-auto mt-8 flex max-w-4xl flex-wrap items-center justify-center gap-x-10 gap-y-4 opacity-60">
      ...
      <section className="mx-auto max-w-4xl px-6 py-20 text-center">
        <h2 className="text-3xl font-bold text-pink md:text-4xl">1000+ Kreator untuk Brand Anda</h2>
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          {CATEGORIES.map((c) => (
            <span key={c} className="rounded-full bg-mint px-5 py-2 text-sm font-medium">{c}</span>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-3xl px-6 py-20">
        <div className="space-y-2">
          {FAQS.map((f, i) => <FaqItem key={i} q={f.q} a={f.a} defaultOpen={i === 1} />)}
        </div>
      </section>
